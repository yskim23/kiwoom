...
class MyBot(Bot):
    def __init__(self, server=None):
        ...
        """ rq_name을 slot을 호출 기준으로 설정하겠다는 의미
            slot을 연결하는 기준을 설정하는 것이기 때문에 한번만 선언하면 됨.
        """
        ...
        # > help(Kiwoom.connect)
        """
            TR요청에 대한 응답 이벤트 on_receive_tr_data 에 시그널(TR요청)인 deposit과 이벤트 발생 후속처리(callback) 메소드
            는 deopsit을 연결. 
            signal=self.deposit은 MyBot 클래스에 메소드로 정의되어 있고, 
            slot=self.server.deposit 은 MyServer 클래스에 정의되어 있음.
            MyServer는 Server를 상속 받은 클래스임.
            서버에 요청하는 메소드는 MyBot에 정의하고, 이벤트에 대한 후처리 슬롯함수(callback함수)는 MyServer에 정의한다.
        ...
        """ chart 3분봉 받아오기 테스트"""
        # self.api.connect('on_receive_tr_data', signal=self.chart_data, slot=self.server.chart_data)
    ...
    def chart_data(self, prev_next='0'):
        print("Bot.chart_data(prev_next) 호출")
        ...
        # TR Data 요청
        self.api.comm_rq_data(rq_name='chart_data', tr_code=tr_code, prev_next=prev_next, scr_no='1010')

        #[필수] on_receive_tr_data 이벤트가 호출 될 때까지 대기
        self.api.loop()

        print("Bot.chart_data(prev_next) 종료")
...
class MyServer(Server):
    ...
    def balance(self, scr_no, rq_name, tr_code, record_name, prev_next):
        ...
        * KOA Studio 참고 가이드
        ...
        """ 아래 코드 설명
            ...
            특정한 종류의 데이터 수신 시 여러개의 데이터 필드에 대해 정보를 저장하고자 할 때 
            아래와 같은 방식으로 초기 구조를 설정할 수 있음.
            dictionary comprehension)을 사용하여 keys 리스트에 있는 각 키를 딕셔너리의 키로 사용하고, 
            각 키에 대응하는 값으로 빈 리스트 list()를 할당
            이렇게 하면 각 데이터 항목에 대해 여러 값을 저장할 수 있는 구조가 생성. 
            예를 들어, '종목번호' 키에 대해 여러 종목의 번호를 리스트 형태로 저장할 수 있게 됨.

            keys = ['종목번호', '종목명', '평가손익', '수익률(%)', '보유수량', '매입가', '현재가']
            data = {key: list() for key in keys}

            # 삼성전자 데이터 추가
            data['종목번호'].append('005930')
            data['종목명'].append('삼성전자')
            data['평가손익'].append('1000000')
            data['수익률(%)'].append('5.0')
            data['보유수량'].append('10')
            data['매입가'].append('50000')
            data['현재가'].append('55000')

            # LG전자 데이터 추가
            data['종목번호'].append('066570')
            data['종목명'].append('LG전자')
            data['평가손익'].append('500000')
            data['수익률(%)'].append('10.0')
            data['보유수량'].append('5')
            data['매입가'].append('80000')
            data['현재가'].append('88000')
    ...
    def chart_data(self, scr_no, rq_name, tr_code, record_name, prev_next):
        
        print('\tServer.chart_data(scr_no, rq_name, tr_code, record_name, prev_next) 호출')

        keys = ['종목코드', '현재가', '시가', '고가', '저가', '거래량', '체결시간', '전일종가']
        data = {key: list() for key in keys}
        cnt = self.api.get_repeat_cnt(tr_code, rq_name)
        ...
        print('\tServer.chart_data(scr_no, rq_name, tr_code, record_name, prev_next) 종료')
...
Multi Data: defaultdict(<class 'list'>,{
    '종목번호': ['A005930'], '종목명': ['삼성전자'], '평가손익': [1234567], 
